...
def test_stationarity_acf_pacf(ts, sample=0.20, maxlag=30, figsize=(15, 10)):
    with plt.style.context(style='bmh'):
        ...
        ts_ax.fill_between(x=dtf_ts.head(sample_size).index, y1=dtf_ts['lower'].head(sample_size),
...
def find_outliers(ts, perc=0.01, figsize=(15, 5)):
    ...
    ax.scatter(x=dtf_outliers[dtf_outliers["outlier"] == 1].index, y=dtf_outliers[dtf_outliers["outlier"] == 1]['ts'],
               color='red')
    ax.grid(True)
    plt.show()
    return dtf_outliers
...
def resistence_support(ts, window=30, trend=False, plot=True, figsize=(15, 5)):
    ...
    local_max_idx = [dtf.iloc[i - window:i + window]['ts'].idxmax() for i in local_max if
                     (i > window) and (i < len(dtf) - window)]
    dtf["max"].loc[local_max_idx] = dtf["ts"].loc[local_max_idx]

    ## mins
    local_min = signal.argrelextrema(rolling.values, np.less)[0]
    local_min_idx = [dtf.iloc[i - window:i + window]['ts'].idxmin() for i in local_min if
                     (i > window) and (i < len(dtf) - window)]
...
def utils_evaluate_ts_model(dtf, conf=0.95, title=None, plot=True, figsize=(20, 13)):
    try:
        ...
        if plot is True:
            ...
            ### forecaster
            dtf[pd.notnull(dtf["model"])][["ts", "model"]].plot(color=["black", "green"], title="Train (obs: " + str(
                len(dtf[pd.notnull(dtf["model"])])) + ")", grid=True, ax=ax1)
            ax1.set(xlabel=None)
            ### test
            dtf[pd.isnull(dtf["model"])][["ts", "forecast"]].plot(color=["black", "red"], title="Test (obs: " + str(
                len(dtf[pd.isnull(dtf["model"])])) + ")", grid=True, ax=ax2)
            ...
            dtf[["residuals", "error"]].plot(ax=ax4, color=["green", "red"], kind='kde', title="Residuals Distribution",
                                             grid=True)
            ax4.axvline(dtf["residuals"].mean(), ls='--', color="green",
                        label="mean: " + str(round(dtf["residuals"].mean(), 2)))
            ...
            print("Test --> Error mean:", np.round(error_mean), " | std:", np.round(error_std),
                  " | mae:", np.round(mae), " | mape:", np.round(mape * 100), "%  | mse:", np.round(mse), " | rmse:",
                  np.round(rmse))
...
def utils_add_forecast_int(dtf, conf=0.95, plot=True, zoom=30, figsize=(15, 5)):
    ...
    if plot is True:
        ...
        dtf.loc[zoom_idx:][["ts", "forecast"]].plot(color=["black", "red"], grid=True, ax=ax1,
                                                    title="Zoom on the last " + str(zoom) + " observations")
        ax1.fill_between(x=dtf.loc[zoom_idx:].index, y1=dtf.loc[zoom_idx:]['lower'], y2=dtf.loc[zoom_idx:]['upper'],
                         color='b', alpha=0.2)
        ax1.set(xlabel=None)
        plt.show()
...
def simulate_rw(ts_train, ts_test, conf=0.95, figsize=(15, 10)):
    ...
    rw = utils_generate_rw(y0=ts_train[0], n=len(ts_train), sigma=diff_ts.std(), ymin=ts_train.min(),
                           ymax=ts_train.max())
    dtf_train = ts_train.to_frame(name="ts").merge(pd.DataFrame(rw, index=ts_train.index, columns=["model"]),
                                                   how='left', left_index=True, right_index=True)

    ## test
    rw = utils_generate_rw(y0=ts_train[-1], n=len(ts_test), sigma=diff_ts.std(), ymin=ts_train.min(),
                           ymax=ts_train.max())
        ...
        for d in damped:
            for ss in seasonal:
                ...
                if combo not in combinations:
                    ...
                    try:
                        ### fit
                        model = smt.ExponentialSmoothing(dtf_fit, trend=t, damped=d, seasonal=ss,
                        ...
                        if pred.isna().sum() == 0:
                            ...
                            dtf_search = dtf_search.append(
                                pd.DataFrame({"combo": [combo], "score": [score], "model": [model]}))
                    except:
                        continue
...
def fit_expsmooth(ts_train, ts_test, trend="additive", damped=False, seasonal="multiplicative", s=None,
                  factors=(None, None, None), conf=0.95, figsize=(15, 10)):
    ## checks
    check_seasonality = "Seasonal parameters: No Seasonality" if (seasonal is None) & (
            s is None) else "Seasonal parameters: " + str(seasonal) + " Seasonality every " + str(
        s) + " observations"
    print(check_seasonality)

    ## train
    model = smt.ExponentialSmoothing(ts_train, trend=trend, damped=damped, seasonal=seasonal, seasonal_periods=s).fit(
    ...
    ## evaluate
    dtf = dtf_train.append(dtf_test)
    alpha, beta, gamma = round(model.params["smoothing_level"], 2), round(model.params["smoothing_slope"], 2), round(
        model.params["smoothing_seasonal"], 2)
...
def tune_arima_model(ts_train, s=7, val_size=0.2, max_order=(3, 1, 3), seasonal_order=(1, 1, 1), scoring=None, top=None,
                     figsize=(15, 5)):
    ## split
    dtf_fit, dtf_val = model_selection.train_test_split(ts_train, test_size=val_size, shuffle=False)
    dtf_fit, dtf_val = dtf_fit.to_frame(name="ts"), dtf_val.to_frame(name="ts")
        ...
        for d in ds:
            for q in qs:
                for P in Ps:
                    for D in Ds:
                        for Q in Qs:
                            combo = "(" + str(p) + "," + str(d) + "," + str(q) + ")x(" + str(P) + "," + str(
                                D) + "," + str(Q) + ")"
                            if combo not in combinations:
                                combinations.append(combo)
                            try:
...
def fit_sarimax(ts_train, ts_test, order=(1, 0, 1), seasonal_order=(1, 0, 1), s=7, exog_train=None, exog_test=None,
                conf=0.95, figsize=(15, 10)):
    ## checks
    check_trend = "Trend parameters: No differencing" if order[1] == 0 else "Trend parameters: d=" + str(order[1])
    print(check_trend)
    check_seasonality = "Seasonal parameters: No Seasonality" if (s == 0) & (
            np.sum(seasonal_order[0:2]) == 0) else "Seasonal parameters: Seasonality every " + str(
        s) + " observations"
    print(check_seasonality)
    check_exog = "Exog parameters: Not given" if (exog_train is None) & (
            exog_test is None) else "Exog parameters: number of regressors=" + str(exog_train.shape[1])
    print(check_exog)

    ## train
    model = smt.SARIMAX(ts_train, order=order, seasonal_order=seasonal_order + (s,), exog=exog_train,
...
def fit_garch(ts_train, ts_test, order=(1, 0, 1), seasonal_order=(0, 0, 0, 0), exog_train=None, exog_test=None,
              figsize=(15, 10)):
    ## train
    arima = smt.SARIMAX(ts_train, order=order, seasonal_order=seasonal_order, exog=exog_train,
                        enforce_stationarity=False, enforce_invertibility=False).fit()
    garch = arch.arch_model(arima.resid, p=order[0], o=order[1], q=order[2], x=exog_train, dist='StudentsT', power=2.0,
...
###############################################################################
# '''
# Plot loss and metrics of keras forecaster.
# '''


# def utils_plot_keras_training(forecaster):
#     metrics = [k for k in forecaster.history.keys() if ("loss" not in k) and ("val" not in k)]
#     fig, ax = plt.subplots(nrows=1, ncols=2, sharey=True, figsize=(15, 3))

#     ## forecaster
#     ax[0].set(title="Training")
#     ax11 = ax[0].twinx()
#     ax[0].plot(forecaster.history['loss'], color='black')
#     ax[0].set_xlabel('Epochs')
#     ax[0].set_ylabel('Loss', color='black')
#     for metric in metrics:
#         ax11.plot(forecaster.history[metric], label=metric)
...
#     ax22 = ax[1].twinx()
#     ax[1].plot(forecaster.history['val_loss'], color='black')
#     ax[1].set_xlabel('Epochs')
#     ax[1].set_ylabel('Loss', color='black')
#     for metric in metrics:
#         ax22.plot(forecaster.history['val_' + metric], label=metric)
...
# def fit_lstm(ts_train, ts_test, model, exog=None, s=20, epochs=100, conf=0.95, figsize=(15, 5), batch_size=2):
#     ## check
...
#     forecaster = model.fit(x=X_train, y=y_train, batch_size=batch_size, epochs=epochs, shuffle=True, verbose=verbose,
#                          validation_split=0.3)
#     dtf_train = ts_train.to_frame(name="ts")
#     dtf_train["model"] = utils_fitted_lstm(ts_train, forecaster.model, scaler, exog)
#     dtf_train["model"] = dtf_train["model"].fillna(method='bfill')

#     ## test
#     last_s_obs = ts_train[-s:]
#     preds = utils_predict_lstm(last_s_obs, forecaster.model, scaler, pred_ahead=len(ts_test), exog=None)
...
#     forecaster = model.fit(x=X, y=y, batch_size=1, epochs=epochs, shuffle=True, verbose=0, validation_split=0.3)
#     dtf = ts.to_frame(name="ts")
#     dtf["model"] = utils_fitted_lstm(ts, forecaster.model, scaler, None)
...
#     ## forecast
#     last_s_obs = ts[-s:]
#     preds = utils_predict_lstm(last_s_obs, forecaster.model, scaler, pred_ahead=len(index), exog=None)
#     dtf = dtf.append(pd.DataFrame(data=preds, index=index, columns=["forecast"]))
...
def forecast_prophet(dtf, model=None, pred_ahead=None, end=None, freq="D", conf=0.95, zoom=30, figsize=(15, 5)):
    ...
    dtf_prophet = model.predict(dtf_prophet)
    dtf = dtf.merge(dtf_prophet[["ds", "yhat"]], how="left").rename(columns={'yhat': 'model', 'y': 'ts'}).set_index(
        "ds")
...
def custom_model(ts, pred_ahead, trend=False, seasonality_types=None, level_window=1, sup_res_windows=(None, None),
                 floor_cap=(False, False), plot=True, figsize=(15, 5)):
...
def fit_custom_model(ts_train, ts_test, trend=False, seasonality_types=None, level_window=None,
                     sup_res_windows=(None, None), floor_cap=(False, False), conf=0.95, figsize=(15, 10)):
    ## train / test
    fitted_values, preds = custom_model(ts_train, pred_ahead=len(ts_test), trend=trend,
                                        seasonality_types=seasonality_types,
                                        level_window=level_window, sup_res_windows=sup_res_windows, floor_cap=floor_cap,
                                        plot=False)
...
def forecast_custom_model(ts, trend=False, seasonality_types=None, level_window=None, sup_res_windows=(None, None),
                          floor_cap=(False, False), pred_ahead=None, end=None, freq="D", conf=0.95, zoom=30,
                          figsize=(15, 5)):
    ## fit / forecast
    index = utils_generate_indexdate(start=ts.index[-1], end=end, n=pred_ahead, freq=freq)
...
def plot_multiple_ts(dtf, idxs, color=None, lst_colors=None, lst_highlights=[], legend=False, title=None,
                     figsize=(15, 5)):
    data = dtf.copy()
...
def clustering_multiple_ts(dtf, idxs, k=None, top=None, figsize=(10, 5)):
    ...
    ## plot freq
    ax = dtf_c["cluster"].value_counts().head(top).sort_values().plot(kind="barh", title="Cluster distribution",
                                                                      figsize=figsize)
